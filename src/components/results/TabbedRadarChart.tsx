...
import { BarChart3, Brain, Palette, AlertCircle } from 'lucide-react';
...
function TabbedRadarChartComponent({ scores, className = '' }: TabbedRadarChartProps) {
  ...
  const getCurrentRadarData = (): RadarDataPoint[] => {
    switch (activeRadar) {
      case 'riasec':
        return riasecData;
      case 'ocean':
...
// Wrap component with error boundary and memoization
export default React.memo(function TabbedRadarChart(props: TabbedRadarChartProps) {
  return (
    <ChartErrorBoundary>
      <TabbedRadarChartComponent {...props} />
    </ChartErrorBoundary>
  );
});